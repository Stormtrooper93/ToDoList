...
function editTask(event) {
    ...
    if(iconContainer.classList.contains('update-item')) {
        ...
        //перевіряємо значення в полі редагування
        // якщо значення пусте, повертаємо повертаємо попереднє значення таски
        if (updatedTask === "") {
            return
        // якщо значення null, таке відбувається при відміні у вікні редагування, то повертаємо попереднє значення таски
        } else if (updatedTask === null){
            return
        // в іншому випадку використовуємо оновлене значення таски
        } else {
            updatedTask
        };
